...
TH = 0.5
USE_CONTOURS = True
RESTRICT_TO_UNS = True

if USE_CONTOURS:
    load_relative_annotations = utils.load_relative_contours
    r2a = utils.r2a_contour
    create_mask = utils.create_mask_from_contours
else:
    load_relative_annotations = utils.load_relative_rectangles
    r2a = utils.r2a_rectangle
    create_mask = utils.create_mask_from_rectangle


for preprocessing in (False, True):
    detection_name = f"exp-incisive-{MDL_INDEX}-{'y' if preprocessing else 'n'}"
    ...
    predicted_annotations_dir = data_dir / "segment" / detection_name / "labels"
    ...
    def process_image(file_name):
        ...
        target_boxes = load_relative_annotations(
            target_annotations_dir / f"{file_name}.txt"
        )
        target_boxes = [r2a(r, w, h) for r in target_boxes]

        try:
            predicted_boxes = load_relative_annotations(
                predicted_annotations_dir / f"{file_name}.txt"
            )
            predicted_boxes = [r2a(r, w, h) for r in predicted_boxes]
        except:
            predicted_boxes = []

        # per lesion
        predicted_mask = create_mask(predicted_boxes, w, h)
        target_masks = [create_mask([tc], w, h) for tc in target_boxes]
        ...
        # per rectangle
        predicted_masks = [create_mask([pb], w, h) for pb in predicted_boxes]
        target_mask = create_mask(target_boxes, w, h)
        rectangles_coverage = [
    ...
    def main():
        ...
        results = Parallel(n_jobs=4)(
            delayed(process_image)(f) for f in tqdm(file_names)
        )
